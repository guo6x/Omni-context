...
describe('decision-benchmark-v2 integrity', () => {
  ...
  test('T15 hash seals, manifests, access log, sealed artifact', () => {
    const sha = (f) => crypto.createHash('sha256').update(fs.readFileSync(path.isAbsolute(f) ? f : path.join(OUT, f))).digest('hex');
    const vManifest = JSON.parse(fs.readFileSync(path.join(OUT, 'validation-manifest.json'), 'utf8'));
    assert.equal(vManifest.files['validation-set.jsonl'].sha256, sha('validation-set.jsonl'), 'val set hash');
    assert.equal(vManifest.files['validation-gold.jsonl'].sha256, sha('validation-gold.jsonl'), 'val gold hash');
    const hbManifest = JSON.parse(fs.readFileSync(path.join(OUT, 'holdback-public-manifest.json'), 'utf8'));
    const hbPlain = fs.existsSync(path.join(OUT, 'holdback-fixtures.jsonl'))
      ? 'holdback-fixtures.jsonl'
      : 'C:/Users/00/.codex/goal18-holdback-custody/holdback-fixtures.jsonl';
    assert.equal(hbManifest.sha256, sha(hbPlain), 'holdback hash');
    assert.ok(hbManifest.seed_hash && /^[0-9a-f]{64}$/.test(hbManifest.seed_hash), 'seed hash format');
    assert.ok(fs.existsSync(path.join(OUT, 'holdback-sealed.bin')), 'sealed artifact exists');
    assert.ok(hbManifest.sealed_artifact.sha256 === sha('holdback-sealed.bin'), 'sealed artifact hash');
    const access = fs.readFileSync(path.join(OUT, 'holdback-access-log.jsonl'), 'utf8').trim().split(/\r?\n/);
