...
`;
function buildExtractionPrompt(userText) {
...
export class LLMExtractorPipeline {
    ...
    _parseResult(content) {
        try {
            ...
            // 验证结构完整性
            return {
                entities: Array.isArray(parsed.entities) ? parsed.entities.filter((e) => e.name && e.type) : [],
                relationships: Array.isArray(parsed.relationships) ? parsed.relationships.filter((r) => r.source && r.target && r.type) : [],
                principles: Array.isArray(parsed.principles) ? parsed.principles.filter((p) => p.title && p.content) : [],
            };
        }
        catch (e) {
