...
class InsightGenerator {
    ...
    async generateInsight(nodes) {
        if (!this.config.apiUrl || nodes.length < 2)
            return null;
...
export class AgentLoop {
    constructor(db) {
        ...
        this.anchorRefreshMs = 30 * 60 * 1000;
        this.db = db;
    ...
    async runCycle() {
        ...
        try {
            ...
            const recentEntities = await this.db.all('SELECT * FROM entities ORDER BY updated_at DESC LIMIT 5');
            ...
            const anchorKey = recentEntities
                ...
                .join('|');
            const now = Date.now();
            if (this.lastAnchorKey === anchorKey &&
