export class CoreMemory {
    ...
    async set(key, value, category = 'general') {
        ...
        await this.db.run(`INSERT INTO core_memory (key, value, category, last_accessed, access_count, summary)
       VALUES (?, ?, ?, datetime('now'), 0, NULL)
       ON CONFLICT(key) DO UPDATE SET
         value = excluded.value,
         category = excluded.category,
         last_accessed = datetime('now'),
         summary = NULL`, [key, valueStr, category]);
    ...
    async compress(category) {
        ...
        for (const item of items) {
            ...
            if (valueStr.length > this.compressionThreshold) {
                ...
                if (wouldSaveSize > 0) {
                    await this.db.run('UPDATE core_memory SET summary = ? WHERE key = ?', [summary, item.key]);
                    itemsProcessed++;
