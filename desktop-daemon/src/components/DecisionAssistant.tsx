...
import { X, Scale, BookOpen, History, ShieldAlert, Loader2, Check, Save } from "lucide-react";
...
export default function DecisionAssistant({ isOpen, onClose, onSelectEntity }: DecisionAssistantProps) {
  ...
  return (
    ...
    >
      <div
        className="glass-panel w-full max-w-5xl border border-cyan-500/20 bg-slate-950/80 rounded-2xl overflow-hidden shadow-[0_0_50px_rgba(6,182,212,0.15)] flex flex-col max-h-[88vh]"
              ...
              <div className="h-full grid grid-cols-3 gap-4">
                {columns.map((col) => {
                  ...
                  return (
                    <div
                      key={col.key}
                      className="flex flex-col min-h-0 border border-white/10 rounded-xl bg-black/20 overflow-hidden"
                    >
                      ...
                      <div className="flex-1 overflow-y-auto p-3 space-y-2">
                        {items.length === 0 ? (
                          <p className="text-xs text-gray-500 text-center py-8">{t(col.emptyKey)}</p>
                        ) : (
                          items.map((item, i) => (
                            <button
                              key={`${col.key}-${item.id}-${i}`}
                              onClick={() => handleItemClick(item.id)}
                              className="w-full text-left p-3 rounded-lg border border-transparent hover:border-cyan-500/20 hover:bg-cyan-500/5 transition-all group"
                            >
                ...
                {!showSaveInput ? (
                  ...
                  <div className="flex flex-col gap-2">
                    <textarea
                      ...
                      className="w-full bg-black/40 border border-emerald-500/30 rounded-lg px-3 py-2 text-white placeholder-gray-500 text-sm resize-none outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/30 transition-all"
                    />
                    <div className="flex items-center gap-2 justify-end">
                      ...
                      <button
                        onClick={handleSave}
                        disabled={!saveConclusion.trim() || isSaving}
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 disabled:cursor-not-allowed text-white text-xs font-medium rounded-md transition-all"
                      >
