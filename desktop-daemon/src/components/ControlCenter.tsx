...
function isSupersededJudgment(record: PlanRecord): boolean {
  return Boolean(record.revision && record.plan.decision_id !== record.revision.current_decision_id);
}
...
export default function ControlCenter({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const requestReopen = (record: PlanRecord) => {
    if (isSupersededJudgment(record)) {
      setError(`Historical judgment ${record.plan.decision_id} has been superseded by ${record.revision?.current_decision_id ?? "the current judgment"}. Reopen is available only from the current judgment.`);
      return;
    }
  ...
  return <div className="fixed inset-0 z-40 flex justify-end bg-black/60" onClick={onClose}>
    <section className="h-full w-full max-w-2xl overflow-y-auto border-l border-cyan-500/20 bg-[#090b12] p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="space-y-4">{records.map((record) => {
        ...
        const superseded = isSupersededJudgment(record);
        const outcomeAllowsReopen = outcome?.status === 'MISMATCH' || outcome?.status === 'INCONCLUSIVE' || outcome?.status === 'VERIFIED';
        const canReopen = outcomeAllowsReopen && !superseded;
        return <article key={plan.plan_id} data-decision-id={plan.decision_id} data-decision-role={superseded ? 'superseded' : 'current'} className={`rounded-xl border p-4 ${superseded ? 'border-slate-500/20 bg-slate-950/20' : 'border-white/10 bg-white/[0.03]'}`}>
          <div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><div className="font-medium text-white">{plan.capability_id}</div>{revision && <span className={`rounded-full px-2 py-1 text-[9px] uppercase tracking-wide ${superseded ? 'bg-slate-500/20 text-slate-300' : 'bg-cyan-500/20 text-cyan-200'}`}>{superseded ? 'Superseded judgment' : 'Current judgment'}</span>}</div><div className="mt-1 font-mono text-[10px] text-gray-500">{plan.plan_id}</div></div><span className={`rounded-full px-2 py-1 text-[10px] uppercase ${pending ? 'bg-amber-500/20 text-amber-300' : ready ? 'bg-emerald-500/20 text-emerald-300' : 'bg-gray-500/20 text-gray-300'}`}>{plan.state}</span></div>
          {superseded && revision && <div className="mt-4 rounded-lg border border-slate-400/20 bg-slate-900/40 p-3 text-xs text-slate-200"><div className="font-medium">Historical judgment · superseded</div><div className="mt-1">This immutable decision remains visible for audit, but the revision chain now points to <span className="font-mono text-[10px] text-cyan-200">{revision.current_decision_id}</span>. This historical card is audit-only; approval, execution, verification, and reopen controls are available only on the current judgment.</div></div>}
