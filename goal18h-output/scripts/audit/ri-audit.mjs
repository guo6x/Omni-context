...
const set = fs.readFileSync(path.join(G18, 'validation-set.jsonl'), 'utf8').trim().split(/\r?\n/).filter(Boolean).map((l) => JSON.parse(l));
if (set.length !== 120) throw new Error('expected 120 validation samples');
...
for (const s of set) {
  ...
  for (const ev of [...(s.evidence.qualified || []), ...(s.evidence.expired || []), ...(s.evidence.conflicting || [])]) {
    for (const sup of ev.supports || []) {
      if (!optIds.has(sup) && !conIds.has(sup) && !decIds.has(sup)) {
        add('RI-02', s, 'evidence[' + ev.id + '].supports', 'references missing id ' + sup + ' (known options=' + [...optIds].join(',') + ' constraints=' + [...conIds].join(',') + ' decisions=' + [...decIds].join(',') + ')', 'ERROR', 'generator builder omitted the referenced constraint/option/decision from the sample');
      }
    }
  }
...
const report = {
  schema_version: 'validation-ri-audit-v1.1',
  scope: 'validation split only (120 samples); holdback plaintext NOT inspected per Goal 18H-R',
  ...
  notes: [
    ...
    'RI-02: evidence[].supports must reference an existing option/constraint/decision id.',
    ...
    'RI-04: TT15 qualified evidence with deleted source_ref is a DESIGNED GHOST-CITATION trap (gold GHOST-CITATION/CASCADE-INVALIDATION); INFO for TT15, ERROR otherwise.',
    'RI-05/06/07/08: bonus referential hygiene checks.'
  ],
  violations
};
fs.writeFileSync(path.join(OUT, 'scripts', 'audit', 'ri-audit-results.json'), JSON.stringify(report, null, 2) + '\n', 'utf8');
console.log(JSON.stringify({ total: violations.length, samples_affected: samplesAffected.size, by_rule: byRule, by_severity: bySeverity }, null, 2));